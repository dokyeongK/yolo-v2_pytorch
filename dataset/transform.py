...
def expand(image, boxes, filler):
    ...
    scale = random.uniform(1, max_scale) # range : 1 ~ 4
...
def random_crop(image, boxes, labels, difficulties):
    ...
    while True:
        ...
        for _ in range(max_trials):
            ...
            left = random.randint(0, original_w - new_w) # 작거나 같으므로.
            right = left + new_w
...
def random_crop_COCO(image, boxes, labels):
    """
    Performs a random crop in the manner stated in the paper. Helps to learn to detect larger and partial objects.
...
def transform_voc(image, boxes, labels, difficulties, mode):
    """
    ...
    :param labels: labels of objects, a tensor of dimensions (n_objects)
    :param difficulties: difficulties of detection of these objects, a tensor of dimensions (n_objects)
    :param mode: one of 'TRAIN' or 'TEST', since different sets of transformations are applied
...
def transform_COCO(image, boxes, labels, mode):
    """
    Apply the transformations above.
    :param image: image, a PIL Image
    :param boxes: bounding boxes in boundary coordinates, a tensor of dimensions (n_objects, 4)
    ...
    if mode == 'TRAIN':
        ...
        # Randomly crop image (zoom in)
        new_image, new_boxes, new_labels = random_crop_COCO(new_image, new_boxes, new_labels)
